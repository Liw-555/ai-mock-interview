...
> **💡 浏览器推荐**：语音面试功能依赖浏览器的 [Web Speech API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API)。**国内用户推荐使用 Microsoft Edge 浏览器**（原生支持，无需代理）；Chrome 需开启代理才能使用语音识别。
...
> **国内用户注意**：如需使用语音面试功能，建议使用 **Microsoft Edge 浏览器**（原生支持 Web Speech API，无需代理）。Chrome 在国内网络环境下需开启代理才能使用语音识别。
...
| 浏览器 | 语音面试 (STT) | 说明 |
|--------|---------------|------|
| **Microsoft Edge** | ✅ 完全支持 | 国内网络环境直接使用，推荐 |
| Chrome | ⚠️ 需代理 | 需开启代理才能调用 Web Speech API |
| Safari | ❌ 不支持 | Web Speech API 兼容性差 |
| Firefox | ❌ 不支持 | 不支持 Web Speech API |

> 语音面试的 STT（语音识别）依赖浏览器内置的 Web Speech API，该服务由 Google 提供。国内网络环境下，**Edge 浏览器可直接使用**，Chrome 需要开启代理。
